...
return new class extends Migration
{
    ...
    {
        // Esta migración ya no es necesaria si users se crea con UUID desde el principio
        // Se mantiene para compatibilidad con instalaciones antiguas
        if (!Schema::hasTable('users')) {
            return; // La tabla users se creará con UUID en la migración anterior
        }
        ...
        if (!empty($columnInfo)) {
            ...
            // Si ya es UUID o char(36), no hacer nada
            if (strpos(strtolower($columnType), 'char') !== false || strpos(strtolower($columnType), 'uuid') !== false) {
                return; // Ya está usando UUID
            }
